...
import { getColors, getTitleColor, getIconsColor } from "./colors.js"

export default function Tooltip({title, text, icon, color, style}) {
    
    const colors = getColors(style)
    const titleColor = getTitleColor(style)
    const iconsColor = getIconsColor(style)
    ...
    return (
        <div className="tooltip-container" style={colors[color]}>
            <div className="tooltip-tip" style={colors[color]}></div>
            <div className="tooltip-icon" style={iconsColor[color]}>{icon}</div>
            <div className="tooltip-inner">
                <h1 className="tooltip-title" style={titleColor[color]}>{title}</h1>
                <p className="tooltip-text" style={colors[color]}>{text}</p>
            </div>
            <button onClick={closeTooltip} className="close-btn">
                <div style={style === "bold" ? {color: "#D8D8D8"} : colors[color]}>
